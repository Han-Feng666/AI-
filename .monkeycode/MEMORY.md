...
[Project Knowledge Summary]
- Date: 2026-09-23
- Context: 第三十轮（v1.4.56）repair 路径门禁漏洞修复 + 用户定义正统系统文形态锁定 + 看点强化（用户实锤：「史官不敢写」无字卷宗显形+烧记忆案例绕过门禁，且系统文变成灵异玄幻、剧情无聊不如传统网文模板）
- Category: Troubleshooting & Debugging
- Instructions:
  - 核心漏洞：/ideas 的 LLM 修复路径（解析失败→repair 成功后）曾直接 `return end({ideas})` 原样放行——首解析失败时题材门禁/载体白名单全部跳过，「史官不敢写」就是从这个漏洞漏出的（新代码白名单单测能拦该案例，端到端却放行=修复路径没过门）。修复：门禁统一收口为 gateIdeas() 闭包，正常解析与修复两条路径都调用，修复产物全剔时同样短路 error。
  - 用户定义的正统系统文四要素（系统题材金手指硬约束，写进 gfSlotNote+GF_POOL 槽位+isSystem 块）：①可视面板——主角能看见并操作的界面；②任务闭环——开局第一个任务带明确奖惩，完成当场得奖励、失败当场受罚；③系统可交流——系统有性格，提示音/对话框与主角一来一回（催促/吐槽/讨价还价）；④金手指描述必须写出"发布任务→完成→结算"循环。GF_POOL 槽位全部改写成该形态。
  - 看点强化：IDEAS_SYSTEM 四问升级五问（⑤第一章兑现——金手指前几章必须完成一次任务→完成→奖励闭环，写不出前几章具体事件=创意没成形）；selling_point 必须具体事件/机制/反差，"智斗张力/悬疑氛围"类抽象空话视为未完成；isSystem 块加看点循环硬约束（高利害首任务/当场兑现奖励/系统人格交锋三选一，全无=废稿）。
  - 词表教训：'气运'会子串碰撞"真气运行"（武侠误报）、'飞升'碰撞"人气飞升"——修法：气运(?!行)、(?<!气)飞升；修仙体系词补 渡劫/炼气/筑基(期|境)/金丹/元婴/结丹/引气入体/御剑，刻意不收 真气/内力/法力（武侠合法词）。残页显形/烧记忆（代价灵异化）进黑名单兜底混合形态。
  - 第四种偷换形态：道具显灵/代价灵异化（卷宗自动显形被删历史、烧记忆换真相）——线索必须系统面板显示、积分结算，禁器物显灵、禁灵异代价替代系统惩罚。
  - E2E SOP（mock 三段式）：mock 按序返回 垃圾→垃圾→违规 JSON（触发 repair），repair 应答后切合法 JSON；测试服 3002+独立 NOVEL_DATA_DIR 隔离真实库，断言场景A=修复产物被剔+error、场景B=经典系统创意 done 放行（/tmp/opencode/e2e_round28.mjs，8 断言）。
  - 测试素材：/tmp/opencode/test_round28.mjs（35 断言）+ 全量 round7-28 sweep 零 fail（round27 四问断言随文案更新为五问）。

[Project Knowledge Summary]
- Date: 2026-09-23
- Context: 第三十一轮（v1.4.57）批内模板收敛修复（用户实锤：勾系统历史穿越架空生成三创意全是"无籍流民小摊起家+系统逼打工否则受罚"——四要素约束被执行成新模板）
- Category: Troubleshooting & Debugging
- Instructions:
  - 过度矫正教训：给生成前约束加硬性要素后，模型会把"合格线"当"剧情公式"逐条打钩——四要素（面板/任务闭环/可交流/结算循环）导致三个创意全是"系统发任务→不完成受罚→主角被鞭策"的同构监工文；约束措辞必须区分"合格线"与"看点公式"，并点名反模板形态（"系统是工具与杠杆，至少一个主角是主动追着奖励搞事"）。
  - "孤家寡人白手起家"这类模板化措辞会把整批创意带偏成同款开局（无籍流民×3）——自由身份分支改为枚举多样开局处境（无籍流民/有名籍小民/行脚客商/退役老卒/投亲访客/落魄书生）+ 强制批内互不相同。
  - 批内撞车门禁：identity 含同 keywords（无籍/流民/流人/死囚）的第二个创意剔除并报「开局身份撞车」——prompt 失守时生成后兜底，防整批同模板换皮。
  - 反套路池冲突 bug：ANTI_TROPE_POOL"金手指不得是面板/属性/等级形态"与系统题材四要素正面冲突，抽中即教模型拆面板——buildAntiTropeBlock(isSystemGenre) 按题材过滤冲突条目，/ideas 与方案层两处调用都要传旗标（方案层用 /系统|金手指|数据流|签到/ 判定）。
  - 六问新增⑥金手指主线咬合：主线核心冲突必须由金手指功能直接推动，金手指只当背景便利=废稿（实锤：汴京报雨人——签到随机奖励对贪墨案主线零贡献）。
  - ④ 灵异代价扩充：抹除他人记忆/让主角"从未存在"的现实改写型惩罚也算灵异代价（实锤：我在永乐开书坊——失败=被世人忘记）。
  - E2E SOP：mock 返回 3 创意（2 个无籍撞车+1 个有名籍）→ 断言撞车剔除消息+保留 2 个（/tmp/opencode/e2e_round29.mjs，4 断言）；测试素材 /tmp/opencode/test_round29.mjs（24 断言）。

[User Instruction Summary]
- Date: 2026-09-23
- Context: 第三十二轮（v1.4.58）用户明确指示精简禁令——"不要太多禁止"
- Instructions:
  - 删除枚举式开局处境列表（"开局处境必须从无籍流民、有名籍的小民…等形态中选取"）——这种枚举反过来限制了灵感，属过度约束。
  - 删除网文模板禁令清单（"禁止套用网文默认模板（如废物嫡子/退婚打脸/签到系统/新手礼包等）"）——列举具体模板名反而暗示模型避开，不如不给清单。
  - 穿越核心铁律只需记住两条：①孤家寡人（刚穿越到异世界时没有亲人朋友家人、没有任何恩情仇恨等既有关系）②身穿（肉身穿越，非魂穿/夺舍/附身）。
  - 安全网性质的 gate 门禁（投亲亲缘剔除、身份撞车、行业赛道撞车）保留——它们不约束生成方向，只兜底剔除违规产物。

[Project Knowledge Summary]
- Date: 2026-09-23
- Context: 第三十二轮（v1.4.58）精简禁令 + 行业赛道分组撞车修复
- Category: Troubleshooting & Debugging
- Instructions:
  - 过度约束教训：枚举开局处境形态（流民/客商/老卒/匠人/书生）本意是促差异化，实际反过来限制了模型想象空间，且枚举项"投亲不遇的访客"自带 bug（投亲即有亲，违反孤家寡人）——用户明确要求删枚举，只留核心铁律+安全网兜底。
