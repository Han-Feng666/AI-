...
要求：构思的故事方向、主角气质与叙述基调向这些风格靠拢。`;
  ...
  // 身份必须是"穿越成古代某身份"。曾因走现实池（现代身份槽位）把穿越挤掉，
  // 生成"跑十年腿的都市人回县城"这种根本没穿越的创意
  const isTransmigration = !isSystem && !isFantasy && genreList.some((g) => /穿越|重生/.test(g));
  ...
  if (isSystem) {
    ...
    ID_POOL = ['现代上班族穿越者', '现代军官/工程师穿越者', '基层小吏', '落魄贵族子弟', '市井游民', '商贾学徒', '边军小卒', '隐姓埋名的读书人'];
    gfLabel = '金手指类型';
...
- 违反题材贴合的创意视为废稿。`;
  ...
  const transmigrationBlock = isTransmigration
