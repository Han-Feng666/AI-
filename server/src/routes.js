...
${blocks.join('\n\n')}`;
  ...
  const isSystem = genreList.some((g) => SYSTEM_KEYWORDS.some((k) => g.includes(k)));
  const isYouth = !isFantasy && genreList.some((g) => YOUTH_KEYWORDS.some((k) => g.includes(k)));
  // 穿越分支：勾穿越/重生但无系统无玄幻——金手指=现代知识/心智（现实型），
  // 身份必须是"穿越后自己谋生的处境"。曾因走现实池（现代身份槽位）把穿越挤掉，
  // 生成"跑十年腿的都市人回县城"这种根本没穿越的创意
  const hasTrans = genreList.some((g) => /穿越|重生/.test(g));
  ...
  if (isSystem) {
    ...
    ID_POOL = hasTrans
      ? ['现代上班族穿越者', '现代工程师/工匠穿越者', '现代历史教师穿越者', '现代医学生穿越者', '现代商人穿越者', '现代刑侦从业者穿越者', '现代厨师穿越者', '现代大学生穿越者']
      : ['现代上班族穿越者', '现代军官/工程师穿越者', '基层小吏', '落魄贵族子弟', '市井游民', '商贾学徒', '边军小卒', '隐姓埋名的读书人'];
    gfLabel = '金手指类型';
  } else if (isTransmigration) {
    // 穿越无系统：金手指=现代知识/心智（现实型优势池保留差异化轴）；
    // 身份完全放开不设槽位（用户指令：身份地位随故事发展获得，开局不打工即可），
    // 只用 transmigrationBlock 管硬底线（身穿/孤家寡人/禁打工/禁职业直译）
    GF_POOL = ['现代知识信息差（历史走向/商业雏形/制度漏洞）', '成年人心智与执行力（对古代同龄人的降维冷静）', '基础数理化与卫生常识的落地应用', '现代管理/组织/营销经验', '现代手艺（复式记账/烹饪/酿造/医卫常识）', '对人性与制度的现代洞察（看穿局中局）', '心理素质与谈判能力（见过大世面不怯场）', '跨时代的审美与创意（做别人没见过的东西）'];
    ID_POOL = [];
    gfLabel = '核心优势类型';
  } else if (isFantasy) {
  ...
  const axisBlock = gfSlots.map((gf, i) => {
    if (needsFreeIdentity) {
      return `创意${i + 1}：${gfLabel}必须属于「${gf}」${gfSlotNote}，主角开局举目无亲、无人可靠，开局身份不预设——身份与地位因契机或随故事发展获得（具体怎么起家由你自由构思，严禁开局处于给人打工受人使唤的状态，严禁把现代职业直译成古代同类营生——商人管军饷国库、刑警翻案、厨师开酒楼这种一一对应缺乏错位趣味，一律废稿）。开局处境必须从无籍流民、有名籍的小民、行脚客商、退役老卒、投亲不遇的访客、落魄书生等形态中选取，且同一批创意的开局处境互不相同，严禁整批都写成"无籍流民小摊起家"的同款开局`;
    }
...
- 正统系统文看点循环（hook/logline/golden_finger 至少体现其一，全部没有=废稿）：开局第一个任务的高利害压力（失败惩罚具体到痛感）、完成任务当场兑现奖励改变处境、系统人格与主角的一来一回对话交锋（毒舌催更/讨价还价/冷酷倒计时）。四要素是合格线而非剧情公式：严禁整批创意都写成"系统逼主角做任务否则受罚"的同构打卡打工文——爽点发动机必须是主角主动玩转系统规则（钻空子/用奖励翻盘/把功能用出新花样），惩罚威胁至多在钩子里作一次开局压力，三个创意里至少一个的主角形态是"主动追着系统奖励搞事"。` : ''}
- 违反题材贴合的创意视为废稿。`;
  // 穿越纯净性块：勾穿越/重生时锁定穿越形态与开局处境。
  // 适用范围：无系统的穿越分支 + 系统+穿越组合（实锤：系统分支曾漏注入，
  // 生成"清河县刑房书吏被推上位"这种根本没有穿越者的本地人故事）。
  // 玄幻+穿越不注入（玄幻世界观里魂穿/夺舍是常见合法设定，由灵感自行声明）。
  // 实锤事故：穿成"原主"后开局被原主关系绑架（替原主送匣/赎原主典当的簪/寻落难的妹妹/
  // 还二十年前恩人的人情）——主角成了原主遗留任务的执行者，穿越者的主动性全无；
...
- 开局孤家寡人：穿越时主角举目无亲，他的处境/身份/营生全部是自己凭本事挣来的，严禁"原主遗留的身份"。
- 严禁开局被遗留关系绑架：不得安排"原主或任何人的恩人/债主/婚约/寻亲任务"——还恩人半贯钱、赎典当的银簪、寻找落难的妹妹这类开局任务全是把穿越者变成别人遗留故事的执行者，一律废稿。主角的目标必须出于他自己的现代意志（用自己的知识/技能在古代谋一条生路、抓住一个商机、避开一段历史灾难）。
- 身份地位是故事发展的结果：开局不预设身份——可以完全白手起家（无身份无名分无家业），身份与地位因某个契机或随剧情推进逐步获得，从无到有的成长弧本身就是看点。两个硬边界：开局严禁处于给别人打工受人使唤的状态（纤夫/伙计/杂役/学徒）；开局严禁坐拥现成权位（王侯将相的嫡子/继承人——那是原主身份的魂穿变体）。
- 严禁现代职业直译成古代同类营生：运动员→船夫力士、体育生→相扑手、鉴定师→当铺朝奉、厨师→酒楼厨子这种一一对应缺乏错位趣味（违反即废稿）。现代优势必须在"表面不相关"的古代场景里发挥错位价值：会计思维破账目迷局、医学常识识破投毒、水利常识改良漕运、谈判能力搅动商战——错位越大，看点越大。
...
请一次构思 ${ideaCount} 个彼此完全不同的小说创意，输出 JSON 数组。`;
  ...
  try {
    ...
    const gateIdeas = (raw) => {
      ...
      // 全是"无籍流民小摊起家"——换皮）→ 保留首个，其余剔除
      const seenIdTpl = new Map();
      list = list.filter((it) => {
        const idText = String(it.protagonist?.identity || '');
        for (const w of ['无籍', '流民', '流人', '死囚']) {
          if (idText.includes(w)) {
            if (seenIdTpl.has(w)) {
              send({ type: 'status', message: `已剔除开局身份撞车的创意「${it.title}」（与「${seenIdTpl.get(w)}」同为${w}开局，整批身份起点必须互不相同）` });
              return false;
            }
            seenIdTpl.set(w, it.title);
            break;
          }
        }
        return true;
    ...
    for (let attempt = 0; attempt <= MAX_AUTO_REGENERATE; attempt++) {
      ...
      try {
        ...
        if (methodMatch) {
          ...
          if (!methodFound && /死(?:了|去|过一次)|猝死|胸(?:口)?痛|加班.{0,6}方案|眼前一黑|白光/.test(full)) {
