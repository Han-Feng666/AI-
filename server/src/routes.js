...
- 开局孤家寡人：刚穿越到异世界时举目无亲——没有亲人、朋友、家人，也没有任何恩情仇恨等既有关系，处境/身份/营生全部是自己凭本事挣来的。
- 严禁开局被遗留关系绑架：不得安排任何恩情、仇恨、婚约、寻亲任务等既有关系——主角的目标必须出于他自己的现代意志（用自己的知识/技能在古代谋一条生路、抓住一个商机、避开一段历史灾难）。
...
请一次构思 ${ideaCount} 个彼此完全不同的小说创意，输出 JSON 数组。`;
  ...
  try {
    ...
    const gateIdeas = (raw) => {
      ...
      // 穿越硬底线检查：开局带亲缘（投亲/寻亲/投奔）= 违反"举目无亲"，直接剔除
      // （实锤 v1.4.58：枚举"投亲不遇的访客"被选中后方案层写出亲人）
      if (hasTrans) {
        list = list.filter((it) => {
          const idText = String(it.protagonist?.identity || '');
          if (/投亲|寻亲|投奔/.test(idText)) {
            send({ type: 'status', message: `已剔除开局带亲缘关系的创意「${it.title}」（${idText}——穿越者必须举目无亲，投亲/寻亲开局即有亲，属硬底线违规）` });
            return false;
          }
          return true;
        });
      ...
      const SCENE_GROUPS = [
        { label: '粮政漕运', words: ['粮仓', '常平仓', '义仓', '漕运', '漕粮', '粮道', '漕粮押运'] },
        { label: '盐政', words: ['盐运', '盐课', '盐引', '盐政'] },
      ...
      list = list.filter((it) => {
        const text = [it.title, it.hook, it.logline].map((s) => String(s || '')).join('｜');
        for (const g of SCENE_GROUPS) {
          if (g.words.some((w) => text.includes(w))) {
            if (seenScene.has(g.label)) {
              send({ type: 'status', message: `已剔除行业赛道撞车的创意「${it.title}」（与「${seenScene.get(g.label)}」同属${g.label}赛道，同一赛道整批只允许一个）` });
              return false;
            }
            seenScene.set(g.label, it.title);
            break;
          }
        }
        return true;
