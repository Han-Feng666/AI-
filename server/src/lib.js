...
// 章尾钩子模板检测（Ending Hook Cliche）：AI 收尾三板斧——
// ① 伪景语收尾："风，更冷了。""夜，深了。"用一个独立成段的短景句假装余韵；
// ② 伪悬念收尾："他知道，真正的考验才刚刚开始""这不是结束，而是开始"；
// ③ 伪剪影收尾："夜色中，他的背影渐渐拉长/消失在……"。
// 只扫章尾 300 字（钩子模板是章级特征，章中段尾偶尔出现可容忍）。
// 阈值：任一模板 ≥2 处，或 3 类合计 ≥2 处 → 触发（章尾是下章的门面，直接重生成级别）。
export function scanEndingHookCliche(text) {
  const s = String(text || '').slice(-300);
  if (s.length < 120) return [];
  const issues = [];
  const detail = [];
  const scene = s.match(/(?:^|[。！？\n"])\s*([^。！？\n"]{0,6}[，,]\s*)?(风|雨|雪|夜|雾|尘|火光|暮色|晨光|月光|阳光|寒意|凉意|寂静|沉默)[^。！？\n"]{0,14}[。！][""]?\s*(?:$|[\n])/g) || [];
  if (scene.length >= 1 && /(?:风|雨|雪|夜|雾|寒意|凉意|寂静|沉默)[^。！？\n]{0,10}(更|愈|越|渐渐|愈发)/.test(s)) detail.push(`伪景语收尾"${(scene[0] || '').trim().slice(0, 18)}"`);
  const fakeSuspense = s.match(/(?:知道|明白|清楚)[^。！？\n]{0,10}(才)?(真正|真正)的?[^。！？\n]{0,8}(才)?刚刚开始|这不是[^。！？\n]{0,10}(结束|终点|结局)[^。！？\n]{0,6}(而是|而是)|真正的?(挑战|考验|较量|战争|游戏|好戏)[^。！？\n]{0,12}(开始|登场|拉开帷幕)/g) || [];
  if (fakeSuspense.length) detail.push(`伪悬念收尾×${fakeSuspense.length}（如"${fakeSuspense[0].slice(0, 20)}"）`);
  const silhouette = s.match(/(背影|身影|轮廓)([^。！？\n]{0,10})(消失|拉长|远去|融进|隐没|没入|模糊)|夜色[^。！？\n]{0,10}(吞没|笼罩|掩去)/g) || [];
  if (silhouette.length >= 1) detail.push(`伪剪影收尾×${silhouette.length}（如"${silhouette[0].slice(0, 20)}"）`);
  if (detail.length) {
    issues.push(`章尾钩子模板（${detail.join('、')}）——章尾用"风更冷了/夜色中背影/真正的考验才刚刚开始"这类模板句假造余韵，是 AI 收尾指纹。真人收尾让剧情本身悬着：一句话没说完的对话、一个刚拿到的反常物件、一个突然响起的声响。请重写章尾最后 2-3 句，用具体的未完成动作或新信息收束，删除全部模板景语`);
  }
  return issues;
}

// 生理反应套话检测（Body Reaction Cliche）：AI 把情绪转译成标准生理件——
// "喉头一紧/心脏漏跳一拍/太阳穴突突直跳/血液仿佛凝固/指尖发凉/后背发凉/头皮发麻"。
// 同一套话全章 ≥3 处说明情绪表达已模板化（单处可容忍，真人也有惯用语）。
export function scanBodyReactionCliche(text) {
  const s = String(text || '');
  if (s.length < 600) return [];
  const issues = [];
  const patterns = [
    { re: /喉头一?(紧|哽|热)|喉咙一?(紧|哽)/g, label: '喉头一紧' },
    { re: /心脏(仿佛|好像)?(漏跳|骤停|停了半?拍)|心口(像被[^。]{0,8})?(攥|揪|堵)/g, label: '心脏漏跳/攥紧' },
    { re: /太阳穴[^。！？\n]{0,6}(突突|怦怦|狂跳)|血管[^。！？\n]{0,4}(突突|暴起)/g, label: '太阳穴突跳' },
    { re: /(血液|浑身的血|全身的血)[^。！？\n]{0,8}(凝固|冻住|变冷|涌上|冲上|上涌|倒流)/g, label: '血液凝固/上涌' },
    { re: /指尖?发(凉|冷|麻)|后背发(凉|麻|紧)|后颈发(凉|紧|麻)|脊背发(凉|寒)|脊背窜过(一股|一阵)凉意/g, label: '指尖/后背发凉' },
    { re: /头皮发麻|汗毛(倒竖|直立|炸起)|寒毛(倒竖|直竖|乍起)/g, label: '头皮发麻/汗毛倒竖' },
    { re: /胃里[^。！？\n]{0,6}(翻江倒海|一阵?(翻腾|抽搐|绞痛)|像被[^。]{0,6}拧)/g, label: '胃部翻腾' },
    { re: /四肢(发软|无力)|双腿一软|膝盖一软|腿肚子转筋/g, label: '腿软' }
  ];
  const detail = [];
  for (const p of patterns) {
    const m = s.match(p.re) || [];
    if (m.length >= 3) detail.push(`${p.label}×${m.length}`);
  }
  if (detail.length) {
    issues.push(`生理反应套话（${detail.join('、')}）——"喉头一紧/心脏漏跳一拍/指尖发凉"是 AI 的情绪生理标准件，全章反复出现就是模板指纹。请把套话替换为与当前场景绑定的具体反应：他正在做什么、手上拿着什么、话说到一半发生了什么——让生理反应长在动作里，而非插入式贴标签`);
  }
  return issues;
}

// 双重比喻检测（Double Simile）：AI 修辞通胀的标志性形态——
// "像刀，又像冰""既像…又像…""像X般…又像Y一样"。单明喻是修辞，双重明喻是堆砌。
// ≥2 处即报（真人极少在一段里叠两个喻体）。
export function scanDoubleSimile(text) {
  const s = String(text || '');
  if (s.length < 600) return [];
  const issues = [];
  const hits = s.match(/(?:像|如同|仿佛|宛如|好似)[^。！？\n，,]{1,10}[，,]?\s*(?:又|也|更)?(?:像|如|似)[^。！？\n]{1,16}|既(?:像|如|似)[^。！？\n，,]{1,12}又(?:像|如|似)[^。！？\n]{1,16}/g) || [];
  if (hits.length >= 2) {
    issues.push(`双重比喻×${hits.length}（如"${hits.slice(0, 2).join('"、"').slice(0, 40)}"）——一句里叠两个喻体是 AI 修辞通胀：单明喻已经是完整修辞，双重明喻说明在凑文采。请每个双重比喻只保留更贴场景的那一个喻体，或干脆改成直白的具体描写`);
  }
  return issues;
}

// 伪精确强调检测（Fake Precision）：AI 用数字+时间词制造郑重感——
// "整整三天三夜""长达十年之久""足足有五分钟""整整一周""足足等了三个小时"。
// 数字本身没错，"整整/长达/足足/将近"+数字的强调组合高频出现就是 AI 腔。
// ≥3 处报（真人在关键处会用一两次）。
export function scanFakePrecision(text) {
  const s = String(text || '');
  if (s.length < 600) return [];
  const issues = [];
  const hits = s.match(/(整整|足足|长达|将近|不下于|足足有)[^。！？\n，,]{0,2}(一|两|三|四|五|六|七|八|九|十|百|千|万|\d)+(多)?(个|天|夜|年|月|小时|分钟|秒|里|米|公里|公斤|斤)/g) || [];
  if (hits.length >= 3) {
    issues.push(`伪精确强调×${hits.length}（如"${hits.slice(0, 3).join('"、"')}"）——"整整三天三夜""长达十年之久"是 AI 制造郑重感的数字模板，用多了像在念报告。请保留最关键的一处，其余改写成具体感受或事件（"等了三天"→"等到窗台上的水仙都谢了"）`);
  }
  return issues;
}
...
export function scanOpeningCliche(text) {
  ...
  // 开头句独立判定：模型会换唤醒源规避组合检测（焦味→公鸡打鸣/鸟叫/锣声），
  // 但"是被…醒的"句式和唤醒动词出现在章节开头本身就是模板，单此一条即触发。
  const head = s.slice(0, 90);
  const openAwaken = /是被[^。！？\n]{1,14}醒的/.test(head) || /(吵醒|熏醒|惊醒|冻醒|呛醒|凉醒|硌醒|震醒|晃醒|疼醒|叫醒)/.test(head);
  if (openAwaken) {
    issues.push('开局即唤醒模板：章节第一句就是"X是被…吵醒/熏醒/冻醒的"或"被X吵醒了"——把唤醒源换成公鸡打鸣、鸟叫、锣声、雷声都一样是模板，AI 味极重。删除唤醒句，直接从醒后的第一个动作写起（人已经在做具体的事：系鞋带/擦刀/掀笼屉），或改用事件中途/对话/反常瞬间切入。若概要指定了"醒来"场景，从睁眼后的第一个行动写起，严禁出现任何"被…唤醒"句式');
  } else if ((feats.includes('感官唤醒') && nonAwaken.length >= 1) || nonAwaken.length >= 3) {
